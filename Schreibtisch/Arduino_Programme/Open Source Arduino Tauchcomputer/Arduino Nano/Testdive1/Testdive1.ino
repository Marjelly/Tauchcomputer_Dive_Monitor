...
const int depthTime=250;
//unsigned long depthTime_t=millis();
...
void loop() {
 ...
 if(depthTime_t<millis()){
  
  readPressureSensor();
  drawDepth();
  calculateMaxDepth();
  drawMaxDepth();
  calculateAscentspeed();
  drawAscentspeed();
  drawTemperature();
  
  depthTime_t=depthTime+millis();
  }
...
void calculateAscentspeed(){
  ascentspeed=(oldDepth-depth)/(depthTime/1000);
  oldDepth=depth;
  }
